...
void mostrarMao(Jogador *jogador) {
    printf("Suas cartas:\n");
    for (int i = 0; i < 3; i++) {
        if (jogador->mao[i].valor != 0) {  // Só mostra cartas válidas
...
void jogarTurno(Jogador *jogador, int jogador_num) {
    ...
    if (opcao == 1) {
        ...
        printf("Escolha uma carta (1-%d): ", availableCards);
        int carta_escolhida;
        scanf("%d", &carta_escolhida);
        int index = carta_escolhida - 1;
        
        jogada = jogador->mao[index];
        printf("Jogador %d jogou [%d %c]\n", jogador_num, jogada.valor, jogada.naipe);
        
        for (int i = index; i < availableCards - 1; i++) {
            jogador->mao[i] = jogador->mao[i + 1];
        }

        jogador->mao[availableCards - 1].valor = 0;
        jogador->mao[availableCards - 1].naipe = ' ';
...
int main() {
    ...
    while (jogador1.pontos < 12 && jogador2.pontos < 12) {
        ...
        mostrarMao(&jogador1);
        jogarTurno(&jogador1, 1);
        
        printf("Mão do Jogador 2:\n");
        mostrarMao(&jogador2);
        jogarTurno(&jogador2, 2);

        compararCartas(jogador1.mao[mao_atual - 1], jogador2.mao[mao_atual - 1]);
